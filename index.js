...
let numBackers = GAMES_JSON.reduce( (acc, game) => {
    return acc + game.backers;
}, 0);

// set the inner HTML using a template literal and toLocaleString to get a number with commas
contributionsCard.innerHTML = Number(`${numBackers}`).toLocaleString('en-US');
...
raisedCard.innerHTML = '$' + Number(`${totalRaised}`).toLocaleString('en-US');
...
unfundedStringDiv.innerHTML = unfundedGamesString;
descriptionContainer.appendChild(unfundedStringDiv);
